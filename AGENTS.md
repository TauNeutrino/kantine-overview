...
>
> **⚠ STANDING INSTRUCTION (wichtig): Nach jedem GitHub Release `git pull --rebase` ausführen, BEVOR weitergearbeitet/gepusht wird.** Der `update-rel-version.yml`-Workflow committet bei jedem Release automatisch `rel-version.json` auf `main` — das lokale Repo liegt danach behind remote, und der nächste Push scheitert mit non-fast-forward. Bewusste Entscheidung (2026-09): KEIN Umbau von Bootloader/Release-CI (Bootloader kann sich nicht selbst updaten, Reinstall durch User nötig — zu großer Eingriff); stattdessen Rebase-Workflow + dieses Memory. Verworfene Alternative: Stable-Pointer als Release-Asset (`releases/latest/download/…`) — GitHub sendet auf Asset-Downloads keinen CORS-Header (live mit Probe-Asset verifiziert), der Ansatz ist tot.
...
- **Versionierung**: Bei jedem Push auf `main` muss die Patch-Nummer in `version.txt` erhöht werden (`v2.0.x` → `v2.0.x+1`). Nur so wird ein neuer Git-Tag erstellt und das Bundle auf jsDelivr CDN ausgeliefert. Ohne Versionssprung liefert der CDN das alte Bundle aus.
- **Bootloader-Update-Erinnerung**: Seit v2.0.6 prüft `checkBootloaderVersion()` in `src/ui_helpers.js` beim Start, ob der Bootloader alt ist. Der Bootloader schreibt seine Version beim Laden via `localStorage._k_boot_ver` (`LS.BOOTLOADER_VERSION_KEY`). Der Check vergleicht gegen `MIN_BOOTLOADER_VERSION` in `src/constants.js` (aktuell `v2.0.5`). Fehlt der Key oder ist die Version < MIN, erscheint ein animiertes ⚠️-Badge. **Zukünftiges Update erzwingen**: `MIN_BOOTLOADER_VERSION` auf die gewünschte neue Version setzen und sicherstellen, dass der Bootloader in `scripts/build.js` diese Version auch schreibt. Alle User mit älterem Bootloader sehen dann die Meldung.
