...
| FR-126 | Das System muss das ab 2026-04 verwendete Menüformat (deutscher Block mit Allergenankern, gefolgt von einem englischen Übersetzungsblock ohne Top-Level-Slash) erkennen und die englischen Übersetzungen den deutschen Gängen zuordnen (gespiegelte Allergen-Codes, Komma-Trennung, `kleine Portion`/`small portion`, inline übersetzte erste Gänge). Lässt sich der englische Block nicht 1:1 aufteilen, muss die deutsche Spalte pro Gang korrekt bleiben und der englische Block vollständig als Übersetzung erhalten bleiben. | Mittel | v2.2.7 (Update v2.2.8) |
| FR-127 | Im Dev-Mode muss die Versionsliste Tags, die zusätzlich als GitHub Release existieren, mit einem Release-Badge kennzeichnen (Tooltip erklärt die Bedeutung). Im Stable-Mode entfällt das Badge, da dort ausschließlich Releases gelistet werden. | Niedrig | v2.2.8 |
| **Bildvorschau (Gerichtsbilder)** | | | |
| FR-123 | Das System muss das Hauptgericht eines Menüs (zweiter Gang; bei Ein-Gang-Menüs dieser Gang) bei Sprachtrennung der Konfidenz 'high' oder 'medium' als auslösenden Link innerhalb der Menübeschreibung anbieten. Im ALL-Sprachmodus und bei template-/niedrigerer Konfidenz (low/fallback) darf kein Link erscheinen. | Mittel | v2.2.1 (Update v2.1.2) |
| FR-124 | Das System muss bei Klick (bzw. Enter) auf den Hauptgericht-Link die Google-Bildersuche zum Gerichtstext in einem neuen Tab öffnen. | Mittel | v2.1.2 |
| FR-125 | Das System muss bei ≥ 500 ms Hover über den Hauptgericht-Link ein Popup mit bis zu 5 Bildern des Gerichts als automatisch wechselndes Karussell (3 s, pausierend bei Hover/Fokus, deaktiviert bei prefers-reduced-motion) anzeigen; Bilder müssen erst bei Öffnung geladen werden (kein Prefetching), primär über eine keyless Proxy-Kette der Google-Bildersuche und bei Ausfall über die Openverse-API; ein dauerhaft sichtbarer „Bei Google öffnen"-Link muss den Fehler-/Leerzustand abfedern. | Mittel | v2.1.2 |
| **Benutzer-Feedback** | | | |
