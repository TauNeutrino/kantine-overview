...
async function loadMenuDataFromAPI() {
    ...
    try {
        ...
        Promise.resolve(/* import() */).then(__webpack_require__.bind(__webpack_require__, 570)).then(uiHelpers => {
...
/* harmony export */   NC: () => (/* binding */ DISH_IMAGE_GOOGLE_TAB_URL),
...
/* harmony export */   h: () => (/* binding */ DISH_IMAGE_FETCH_TIMEOUT_MS),
/* harmony export */   nZ: () => (/* binding */ DISH_IMAGE_WIKIPEDIA_URL),
/* harmony export */   pe: () => (/* binding */ GITHUB_API),
/* harmony export */   q: () => (/* binding */ GIST_PAT),
/* harmony export */   tE: () => (/* binding */ API_BASE),
/* harmony export */   v0: () => (/* binding */ DISH_IMAGE_MAX_RESULTS),
...
/** Dish image search: hover-dwell popup timing, popover sources and cache. */
const DISH_IMAGE_HOVER_MS = 500;
const DISH_IMAGE_CAROUSEL_INTERVAL_MS = 3000;
const DISH_IMAGE_FETCH_TIMEOUT_MS = 10000;
const DISH_IMAGE_MAX_RESULTS = 5;
const DISH_IMAGE_CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1000;
/** Wikipedia article summary (lead image) — direct fetch, CORS-enabled, no proxy needed. */
const DISH_IMAGE_WIKIPEDIA_URL = 'https://de.wikipedia.org/api/rest_v1/page/summary/{q}';
...
const DISH_IMAGE_WORKER_URL = 'https://example.workers.dev';
...
const GIST_ID = 'dummy_id';
const GIST_SALT = 'dummy_salt';
const GIST_PAT = 'VRFaVBprQlNC';
...
const TRANSLATIONS = {
    de: {
        ...
        releaseBadgeTooltip: 'Offizielles GitHub Release (kein reiner Dev-Tag)',
    ...
    en: {
        ...
        releaseBadge: 'Release',
        releaseBadgeTooltip: 'Official GitHub release (not a dev-only tag)',
...
__webpack_require__.d(__webpack_exports__, {
  um: () => (/* binding */ checkBootloaderVersion),
  Ux: () => (/* binding */ checkForUpdates),
  zf: () => (/* binding */ closeDishImageModal),
  Gk: () => (/* binding */ openVersionMenu),
  OR: () => (/* binding */ renderVisibleWeeks),
  showErrorModal: () => (/* binding */ showErrorModal),
  wy: () => (/* binding */ syncMenuItemHeights),
  Mb: () => (/* binding */ updateAlarmBell),
  gJ: () => (/* binding */ updateNextWeekBadge)
});

// UNUSED EXPORTS: createDayCard, fetchVersions, openDishImageModal, openInstallPage, removeCountdown, updateCountdown
...
function buildGoogleImageUrl(query) {
    return constants/* DISH_IMAGE_GOOGLE_TAB_URL */.NC.replace('{q}', encodeURIComponent(query))
}
...
async function fetchDishImages(query, lang, cancelSignal, queryDe) {
    ...
    const attemptSignal = () => {
        const timeoutSignal = AbortSignal.timeout(constants/* DISH_IMAGE_FETCH_TIMEOUT_MS */.h)
    ...
    for (const candidate of queryCandidates(query)) {
        ...
        try {
            const response = await fetch(constants/* DISH_IMAGE_WIKIPEDIA_URL */.nZ.replace('{q}', encodeURIComponent(candidate)), { signal: attemptSignal() })
            if (!response.ok) {
            ...
            if (typeof thumb === 'string' && thumb.startsWith('http')) {
                const images = [{ url: thumb, license: '', creator: summary.title || candidate }].slice(0, constants/* DISH_IMAGE_MAX_RESULTS */.v0)
    ...
    for (const candidate of queryCandidates(query)) {
        if (isCancelled()) return { images: [], source: null }
        try {
            const response = await fetch(constants/* DISH_IMAGE_COMMONS_URL */.Gm.replace('{q}', encodeURIComponent(candidate)), { signal: attemptSignal() })
            if (!response.ok) {
    ...
    // Stage 3: Openverse.
    try {
        const response = await fetch(constants/* DISH_IMAGE_OPENVERSE_URL */.be.replace('{q}', encodeURIComponent(query)), { signal: attemptSignal() })
        if (response.ok) {
            const data = await response.json()
...
function readDishImageCache() {
    try {
        return JSON.parse(localStorage.getItem(constants.LS.DISH_IMAGE_CACHE))
    } catch (e) {
        return null
    }
}
...
function writeDishImageCache(key, source, images) {
    ...
    for (const existingKey of Object.keys(queries)) {
        if (now - queries[existingKey].ts >= constants/* DISH_IMAGE_CACHE_TTL_MS */.yd) delete queries[existingKey]
    }
...
const heatmapLangModel = (0,langModel/* createLangModel */.C)(langModelSeed/* LANG_MODEL_SEED */.x);
...
function startDishImageAdvance() {
    ...
    dishImageIntervalId = setInterval(() => showDishImageSlide(dishImageCarouselIndex + 1), constants/* DISH_IMAGE_CAROUSEL_INTERVAL_MS */.Rx);
}
...
function renderDishImageError(query, isEmpty) {
    ...
    if (!body) return;
    stopDishImageAdvance();
    body.innerHTML = `<p class="dish-image-error-text">${(0,utils/* escapeHtml */.ZD)(isEmpty ? (0,i18n.t)('dishImageNoImages') : (0,i18n.t)('dishImageError'))}</p><a href="${(0,utils/* escapeHtml */.ZD)(buildGoogleImageUrl(query))}" target="_blank" rel="noopener noreferrer" class="dish-image-google-link">${(0,utils/* escapeHtml */.ZD)((0,i18n.t)('dishImageOpenInGoogle'))}</a>`;
    const link = body.querySelector('.dish-image-google-link');
    if (link) link.addEventListener('click', () => stats_tracker/* tracker */.F.increment('dish_image_tab'));
}
...
function renderDishImageCarousel(query, result) {
    ...
    body.querySelector('.dish-image-google-link').addEventListener('click', () => stats_tracker/* tracker */.F.increment('dish_image_tab'));
...
function updateNextWeekBadge() {
    ...
    let nextWeek = state/* currentWeekNumber */.BT + 1;
    let nextYear = state/* currentYear */.vW;
    if (nextWeek > 52) { nextWeek = 1; nextYear++; }

    const nextWeekData = state/* allWeeks */.p_.find(w => w.weekNumber === nextWeek && w.year === nextYear);
        ...
        nextWeekData.days.forEach(day => {
            if (day.items && day.items.length > 0) {
                ...
                day.items.forEach(item => {
                    ...
                    if (state/* orderMap */.L.has(key) && state/* orderMap */.L.get(key).length > 0) hasOrder = true;
                });
    ...
    if (totalDataCount > 0) {
        ...
        if (nextWeekData && nextWeekData.days) {
            nextWeekData.days.forEach(day => {
                day.items.forEach(item => {
                    const nameMatches = (0,actions/* checkHighlight */.BM)(item.name);
                    const descMatches = (0,actions/* checkHighlight */.BM)(item.description);
        ...
        let tooltipParts = [`${daysWithOrders} ${(0,i18n.t)('badgeOrdered')} / ${orderableCount} ${(0,i18n.t)('badgeOrderable')} / ${totalDataCount} ${(0,i18n.t)('badgeTotal')}`];
        if (highlightCount > 0) {
            tooltipParts.push(`${highlightCount} ${(0,i18n.t)('badgeHighlights')}`);
        }
        ...
        if (monThuOrderableNoOrder > 0) {
            ...
            if (!localStorage.getItem(storageKey)) {
                ...
                (0,actions/* showToast */.P0)((0,i18n.t)('newMenuDataAvailable'), 'info');
                if (Notification.permission === 'granted') {
                    new Notification('Kantine Wrapper', {
                        body: (0,i18n.t)('newMenuDataAvailable'),
                        icon: '🍽️'
                    });
        ...
        btnNextWeek.title = (0,i18n.t)('nextWeekTooltipDefault');
        btnNextWeek.classList.remove('new-week-available');
    }
}
...
function renderVisibleWeeks() {
    ...
    let targetWeek = state/* currentWeekNumber */.BT;
    let targetYear = state/* currentYear */.vW;
    ...
    const allDays = state/* allWeeks */.p_.flatMap(w => w.days || []);
    const daysInTargetWeek = allDays.filter(day => {
        const d = new Date(day.date);
        return (0,utils/* getISOWeek */.sn)(d) === targetWeek && (0,utils/* getWeekYear */.Ao)(d) === targetYear;
    });

    if (daysInTargetWeek.length === 0) {
        menuContainer.innerHTML = `
            <div class="empty-state">
                <p>${(0,i18n.t)('noMenuData')} ${targetWeek} (${targetYear}).</p>
                <small>${(0,i18n.t)('noMenuDataHint')}</small>
            </div>`;
        return;
    }
    ...
    const weekTitle = state/* displayMode */.sw === 'this-week' ? (0,i18n.t)('thisWeek') : (0,i18n.t)('nextWeek');
    headerWeekInfo.innerHTML = `
        <div class="header-week-title">${weekTitle}</div>
        <div class="header-week-subtitle">${(0,i18n.t)('weekLabel')} ${targetWeek} • ${targetYear}</div>`;
...
function createDayCard(day) {
    ...
    if (day.items) {
        day.items.forEach(item => {
            ...
            const orders = state/* orderMap */.L.get(orderKey) || [];
    ...
    const hasAnyOrder = day.items && day.items.some(item => {
        const articleId = item.articleId || parseInt(item.id.split('_')[1]);
        const key = `${day.date}_${articleId}`;
        return state/* orderMap */.L.has(key) && state/* orderMap */.L.get(key).length > 0;
    });
    ...
    header.innerHTML = `
        <div class="day-header-left">
            <span class="day-name">${(0,utils/* translateDay */.FS)(day.weekday)}</span>
    ...
    const sortedItems = [...day.items].sort((a, b) => {
        if (isToday) {
            ...
            const aOrdered = state/* orderMap */.L.has(`${day.date}_${aId}`);
            const bOrdered = state/* orderMap */.L.has(`${day.date}_${bId}`);
    ...
    sortedItems.forEach(item => {
        ...
        const articleId = item.articleId || parseInt(item.id.split('_')[1]);
        const orderKey = `${day.date}_${articleId}`;
        const orderIds = state/* orderMap */.L.get(orderKey) || [];
        const orderCount = orderIds.length;

        const dm = localStorage.getItem(constants.LS.DEV_MODE) === 'true';
        ...
        let statusBadge = '';
        if (!item.available) {
            statusBadge = `<span class="badge sold-out">${(0,i18n.t)('soldOut')}</span>`;
        } else if (isLowStock) {
            statusBadge = dm
                ? `<span class="badge available-low">${(0,i18n.t)('available')} (${item.availableAmount})</span>`
                : `<span class="badge available-low">${(0,i18n.t)('lowStock')}</span>`;
        } else if (!item.amountTracking) {
            statusBadge = `<span class="badge available">${(0,i18n.t)('available')}</span>`;
        } else if (dm) {
            statusBadge = `<span class="badge available">${(0,i18n.t)('available')} (${item.availableAmount})</span>`;
        } else {
            statusBadge = `<span class="badge available">${(0,i18n.t)('available')}</span>`;
        }

        const split = (0,utils/* splitLanguage */.dk)(item.description || '');
        const lbl = split.label || 'fallback';
        
        let dTitle = '';
        if (lbl !== 'high' && lbl !== 'template') {
            dTitle = ` title="${(0,utils/* escapeHtml */.ZD)(item.description || '')}"`;
        }
        ...
        if (dm && lbl !== 'high') {
            ...
            const chars = affinities.map(({char, affinity}) => {
                ...
                return `<span class="heatmap-char" style="color: ${color}">${(0,utils/* escapeHtml */.ZD)(char)}</span>`;
            }).join('');
        ...
        if (orderCount > 0) {
            ...
            orderedBadge = `<span class="badge ordered"><span class="material-icons-round">check_circle</span> ${(0,i18n.t)('ordered')}${countBadge}</span>`;
        ...
        const isFlagged = state/* userFlags */.BY.has(flagId);
        if (isFlagged) {
            itemEl.classList.add(item.available ? 'flagged-available' : 'flagged-sold-out');
        }

        const matchedTags = [...new Set([...(0,actions/* checkHighlight */.BM)(item.name), ...(0,actions/* checkHighlight */.BM)(item.description)])];
        ...
        if (state/* authToken */.gX && !isPastCutoff) {
            const flagIcon = isFlagged ? 'notifications_active' : 'notifications_none';
            const flagClass = isFlagged ? 'btn-flag active' : 'btn-flag';
            const flagTitle = isFlagged ? (0,i18n.t)('flagDeactivate') : (0,i18n.t)('flagActivate');
            if (!item.available || isFlagged) {
                flagButton = `<button class="${flagClass}" data-date="${day.date}" data-article="${articleId}" data-name="${(0,utils/* escapeHtml */.ZD)(item.name)}" data-cutoff="${day.orderCutoff}" title="${flagTitle}"><span class="material-icons-round">${flagIcon}</span></button>`;
            }

            if (item.available) {
                if (orderCount > 0) {
                    orderButton = `<button class="btn-order btn-order-compact" data-date="${day.date}" data-article="${articleId}" data-name="${(0,utils/* escapeHtml */.ZD)(item.name)}" data-price="${item.price}" data-desc="${(0,utils/* escapeHtml */.ZD)(item.description || '')}" title="${(0,utils/* escapeHtml */.ZD)(item.name)} – ${(0,i18n.t)('orderAgainTooltip')}"><span class="material-icons-round">add</span></button>`;
                } else {
                    orderButton = `<button class="btn-order" data-date="${day.date}" data-article="${articleId}" data-name="${(0,utils/* escapeHtml */.ZD)(item.name)}" data-price="${item.price}" data-desc="${(0,utils/* escapeHtml */.ZD)(item.description || '')}" title="${(0,utils/* escapeHtml */.ZD)(item.name)} – ${(0,i18n.t)('orderTooltip')}"><span class="material-icons-round">add_shopping_cart</span> ${(0,i18n.t)('orderButton')}</button>`;
                }
            }

            if (orderCount > 0) {
                const cancelIcon = orderCount === 1 ? 'close' : 'remove';
                const cancelTitle = orderCount === 1 ? (0,i18n.t)('cancelOrder') : (0,i18n.t)('cancelOneOrder');
                cancelButton = `<button class="btn-cancel" data-date="${day.date}" data-article="${articleId}" data-name="${(0,utils/* escapeHtml */.ZD)(item.name)}" title="${cancelTitle}"><span class="material-icons-round">${cancelIcon}</span></button>`;
            }
        }

        let tagsHtml = '';
        if (matchedTags.length > 0) {
            const badges = matchedTags.reduce((acc, t) => acc + `<span class="tag-badge-small"><span class="material-icons-round" style="font-size:10px;margin-right:2px">star</span>${(0,utils/* escapeHtml */.ZD)(t)}</span>`, '');
        ...
        if (orderBtn) {
            orderBtn.addEventListener('click', (e) => {
                ...
                (0,actions/* placeOrder */.wH)(btn.dataset.date, parseInt(btn.dataset.article), btn.dataset.name, parseFloat(btn.dataset.price), btn.dataset.desc || '')
        ...
        if (cancelBtn) {
            cancelBtn.addEventListener('click', (e) => {
                ...
                const btn = e.currentTarget;
                btn.disabled = true;
                (0,actions/* cancelOrder */.N4)(btn.dataset.date, parseInt(btn.dataset.article), btn.dataset.name)
        ...
        if (flagBtn) {
            flagBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                const btn = e.currentTarget;
                (0,actions/* toggleFlag */.PQ)(btn.dataset.date, parseInt(btn.dataset.article), btn.dataset.name, btn.dataset.cutoff);
            });
        }
...
async function fetchReleaseTagSet() {
    try {
        const resp = await fetch(`${constants/* GITHUB_API */.pe}/releases?per_page=100`, { headers: (0,api/* githubHeaders */.O)() });
...
async function fetchVersions(devMode) {
    const endpoint = devMode
        ? `${constants/* GITHUB_API */.pe}/tags?per_page=20`
        : `${constants/* GITHUB_API */.pe}/releases?per_page=20`;
    ...
    const storedEtag = devMode ? null : localStorage.getItem(constants.LS.VERSION_ETAG);
    const resp = await fetch(endpoint, { headers: (0,api/* githubHeaders */.O)(storedEtag) });
    ...
    if (!devMode) {
        ...
        if (newEtag) localStorage.setItem(constants.LS.VERSION_ETAG, newEtag);
    }
    ...
    return data.map(item => {
        ...
        return {
            ...
            rawUrl: `${constants/* RAW_INSTALLER_BASE */.IY}/${tag}/dist/install.html`,
            // GitHub file browser URL: opened directly in new tab
            githubUrl: `${constants/* GITHUB_FILE_BASE */.fK}/${tag}/dist/install.html`,
            body: item.body || '',
...
async function checkForUpdates() {
    const currentVersion = 'v2.2.7';
    const devMode = localStorage.getItem(constants.LS.DEV_MODE) === 'true';

    // Cache-first: use cached versions if ≤1h old to avoid hitting GitHub API rate limit
    const cachedRaw = localStorage.getItem(constants.LS.VERSION_CACHE);
    if (cachedRaw) {
        try {
            ...
            if (cached && cached.timestamp && cached.devMode === devMode && cached.versions && cached.versions.length) {
                ...
                if (age < 60 * 60 * 1000) {
                    ...
                    if ((0,utils/* isNewer */.U4)(latest, currentVersion)) {
    ...
    try {
        ...
        localStorage.setItem(constants.LS.VERSION_CACHE, JSON.stringify({
            timestamp: Date.now(), devMode, versions
        }));

        const latest = versions[0].tag;

        if (!(0,utils/* isNewer */.U4)(latest, currentVersion)) return;
...
function checkBootloaderVersion() {
    const bootVer = localStorage.getItem(constants.LS.BOOTLOADER_VERSION_KEY);
    if (bootVer && !(0,utils/* isNewer */.U4)(constants/* MIN_BOOTLOADER_VERSION */.w$, bootVer)) {
    ...
    badge.innerHTML = '⚠️';
    badge.title = (0,i18n.t)('bootloaderUpdateTooltip');

    const tooltip = document.createElement('div');
    tooltip.className = 'bootloader-warning-tooltip hidden';
    tooltip.innerHTML = `
        <div class="bootloader-warning-text">${(0,i18n.t)('bootloaderUpdateTooltip')}</div>
        <button class="btn-install-bootloader">${(0,i18n.t)('bootloaderUpdateLink')}</button>
    `;
    ...
    tooltip.querySelector('.btn-install-bootloader').addEventListener('click', () => {
        const installerUrl = `${constants/* RAW_INSTALLER_BASE */.IY}/${constants/* CLIENT_VERSION */.fZ}/dist/install.html`;
...
function openVersionMenu() {
    ...
    async function loadVersions(forceRefresh) {
        ...
        function renderVersionsList(versions, devMode) {
            ...
            versions.forEach(v => {
                ...
                const isNew = (0,utils/* isNewer */.U4)(v.tag, currentVersion);
                ...
                const releaseBadge = (devMode && v.isRelease)
                    ? `<span class="badge-release" title="${(0,utils/* escapeHtml */.ZD)((0,i18n.t)('releaseBadgeTooltip'))}">${(0,utils/* escapeHtml */.ZD)((0,i18n.t)('releaseBadge'))}</span>`
                    : '';

                li.innerHTML = `
                    <div class="version-info">
                        <strong>${(0,utils/* escapeHtml */.ZD)(v.tag)}</strong>
                        ${releaseBadge}
                        ${badge}
                    </div>
                    <div class="version-actions">
                        <button class="btn-install-raw"
                            data-raw-url="${(0,utils/* escapeHtml */.ZD)(v.rawUrl)}"
                            title="${(0,utils/* escapeHtml */.ZD)(v.tag)} installieren (laedt Install-Seite aus GitHub Raw-Content)">
                            Installieren
                        </button>
                        <a href="${(0,utils/* escapeHtml */.ZD)(v.githubUrl)}" target="_blank" class="btn-github-link"
                            title="${(0,utils/* escapeHtml */.ZD)(v.tag)} auf GitHub ansehen">
                            &rarr; Github
                        </a>
                    </div>
                `;
        ...
        try {
            ...
            container.innerHTML = `<p style="color:#e94560;">Fehler: ${(0,utils/* escapeHtml */.ZD)(e.message)}</p>`;
        }
    }
    ...
    devToggle.onchange = async () => {
        if (devToggle.checked) {
            const promptText = state/* langMode */.Kl === 'en' ? 'Dev-Mode password:' : 'Dev-Mode Passwort:';
            ...
            const hashHex = Array.from(new Uint8Array(hashBuffer))
                ...
                .join('');
            if (hashHex !== constants/* DEV_MODE_PW_HASH */.Z7) {
                devToggle.checked = false;
                localStorage.setItem(constants.LS.DEV_MODE, 'false');
                console.warn('[Kantine] Dev-Mode activation rejected: wrong password.');
                return;
            }
            localStorage.setItem(constants.LS.DEV_MODE, 'true');
        } else {
            localStorage.setItem(constants.LS.DEV_MODE, 'false');
        }
        localStorage.removeItem(constants.LS.VERSION_CACHE);
        localStorage.removeItem(constants.LS.VERSION_ETAG);
        loadVersions(true);
        renderVisibleWeeks();
    };
}
...
function updateCountdown() {
    if (!state/* authToken */.gX || !state/* currentUser */.Ny) {
        removeCountdown();
        return;
    }
    ...
    let hasOrder = false;
    for (const key of state/* orderMap */.L.keys()) {
    ...
    countdownEl.innerHTML = `<span>${(0,i18n.t)('orderDeadline')}:</span> <strong>${diffHrs}h ${diffMins}m</strong>`;
...
function updateAlarmBell() {
    ...
    for (const wk of state/* allWeeks */.p_) {
        if (!wk.days) continue;
        for (const d of wk.days) {
            if (!d.items) continue;
            for (const item of d.items) {
                if (item.available && state/* userFlags */.BY.has(item.id)) {
                    anyAvailable = true;
                    break;
                }
            }
    ...
    const lastCheckedStr = localStorage.getItem(constants.LS.LAST_CHECKED);
    const flaggedLastCheckedStr = localStorage.getItem(constants.LS.FLAGGED_LAST_CHECKED);
    ...
    if (latestTime === 0) {
        const now = new Date().toISOString();
        localStorage.setItem(constants.LS.LAST_CHECKED, now);
        latestTime = new Date(now).getTime();
    }

    timeStr = (0,utils/* getRelativeTime */.gs)(new Date(latestTime));

    bellBtn.title = `${(0,i18n.t)('alarmLastChecked')}: ${timeStr}`;
...
const SHARED_LANG_MODEL = (0,langModel/* createLangModel */.C)(langModelSeed/* LANG_MODEL_SEED */.x);
...
function getLocalizedText(text, split) {
    ...
    if (state/* langMode */.Kl === 'en') return s.en || s.raw;
    return s.de || s.raw;
}
            ...
            <div class="header-content">
                <div class="brand">
                    ...
                    <div class="header-left">
                        <h1>Kantinen Übersicht <small class="version-tag" style="font-size: 0.6em; opacity: 0.7; font-weight: 400; cursor: pointer;" title="Klick für Versionsmenü">v2.2.7</small></h1>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div style="margin-bottom: 1rem;">
                        <strong>Aktuell:</strong> <span id="version-current">v2.2.7</span>
                    </div>
...
// EXTERNAL MODULE: ./src/actions.js
var actions = __webpack_require__(367);
// EXTERNAL MODULE: ./src/ui_helpers.js + 1 modules
var ui_helpers = __webpack_require__(570);
// EXTERNAL MODULE: ./src/constants.js
var constants = __webpack_require__(521);
// EXTERNAL MODULE: ./src/api.js
var api = __webpack_require__(672);
...
function updateUILanguage() {
    ...
    if (btnDishImageClose) {
        const closeLabel = (0,i18n.t)('dishImageClose');
...
function bindEvents() {
    ...
    const btnDishImageClose = document.getElementById('btn-dish-image-close');
    if (btnDishImageClose) {
        btnDishImageClose.addEventListener('click', () => (0,ui_helpers/* closeDishImageModal */.zf)());
    }
    window.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') (0,ui_helpers/* closeDishImageModal */.zf)();
    });
...
if (!window.__KANTINE_LOADED) {
    ...
    stats_tracker/* tracker */.F.incrementCategory('version', 'v2.2.7');
