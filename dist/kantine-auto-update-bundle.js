...
/* harmony export */   _F: () => (/* binding */ DISH_IMAGE_PROXY_CHAIN),
/* harmony export */   av: () => (/* binding */ DISH_IMAGE_GOOGLE_SCRAPE_URL),
...
const CLIENT_VERSION = 'v2.1.9';
const COMMIT_HASH = '9f21889';
...
const DISH_IMAGE_WORKER_URL = '';
/** Google Programmable Search engine ID (cx) — embeds real Google image results in the popover; empty string disables the mode. */
const DISH_IMAGE_CSE_ID = 'f68b6c547565548cc';
const DISH_IMAGE_GOOGLE_SCRAPE_URL = 'https://www.google.com/search?q={q}&tbm=isch&hl={hl}&gl=at&ijn=0';
const DISH_IMAGE_OPENVERSE_URL = 'https://api.openverse.org/v1/images/?q={q}&page_size=5';
const DISH_IMAGE_PROXY_CHAIN = [{ name: 'allorigins-raw', template: 'https://api.allorigins.win/raw?url={url}' }, { name: 'codetabs', template: 'https://api.codetabs.com/v1/proxy?quest={url}' }, { name: 'allorigins-get', template: 'https://api.allorigins.win/get?url={url}' }, { name: 'corsproxy-io', template: 'https://corsproxy.io/?key=9949ccab&url={url}' }];
 ...
 */
function extractImageThumbs(html) {
    ...
    return images.slice(0, constants/* DISH_IMAGE_MAX_RESULTS */.v0)
}
...
async function fetchDishImages(query, lang, cancelSignal) {
    ...
    const scrapeUrl = constants/* DISH_IMAGE_GOOGLE_SCRAPE_URL */.av
        .replace('{q}', encodeURIComponent(query))
        .replace('{hl}', lang === 'en' ? 'en' : 'de')
    const attempts = constants/* DISH_IMAGE_PROXY_CHAIN */._F.map(proxy => {
    ...
    if (isCancelled()) return { images: [], source: null }
    for (let i = 0; i < constants/* DISH_IMAGE_PROXY_CHAIN */._F.length; i++) {
        if (settled[i].length >= 1) {
            writeDishImageCache(key, 'google', settled[i])
            note('log', `fertig in ${Date.now() - startedAt}ms — ${settled[i].length} Bilder (Quelle: Google via ${constants/* DISH_IMAGE_PROXY_CHAIN */._F[i].name})`)
...
async function fetchVersions(devMode) {
    ...
    const storedEtag = localStorage.getItem(constants.LS.VERSION_ETAG);
            ...
            <div class="header-content">
                <div class="brand">
                    ...
                    <div class="header-left">
                        <h1>Kantinen Übersicht <small class="version-tag" style="font-size: 0.6em; opacity: 0.7; font-weight: 400; cursor: pointer;" title="Klick für Versionsmenü">v2.1.9</small></h1>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div style="margin-bottom: 1rem;">
                        <strong>Aktuell:</strong> <span id="version-current">v2.1.9</span>
                    </div>
...
if (!window.__KANTINE_LOADED) {
    ...
    stats_tracker/* tracker */.F.incrementCategory('version', 'v2.1.9');
