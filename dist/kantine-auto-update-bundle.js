 ...
 */
async function fetchDishImages(query, lang) {
    ...
    for (const proxy of constants/* DISH_IMAGE_PROXY_CHAIN */._F) {
        const attemptStartedAt = Date.now()
        try {
            const response = await fetch(proxy.template.replace('{url}', encodeURIComponent(scrapeUrl)), { signal: AbortSignal.timeout(constants/* DISH_IMAGE_FETCH_TIMEOUT_MS */.h) })
            if (!response.ok) {
    ...
    try {
        ...
        const response = await fetch(constants/* DISH_IMAGE_OPENVERSE_URL */.be.replace('{q}', encodeURIComponent(query)), { signal: AbortSignal.timeout(constants/* DISH_IMAGE_FETCH_TIMEOUT_MS */.h) })
        if (response.ok) {
            const data = await response.json()
...
function openDishImageModal(query, linkEl) {
    ...
    document.getElementById('dish-image-title').textContent = (0,i18n.t)('dishImageModalTitle');
    document.getElementById('dish-image-body').innerHTML = '<div class="skeleton dish-image-skeleton"></div>'.repeat(3) + `<p class="dish-image-loading-text">${(0,utils/* escapeHtml */.ZD)((0,i18n.t)('dishImageLoading'))}</p>`;
...
function renderDishImageCarousel(query, result) {
    ...
    const sourceLabel = (0,i18n.t)(result.source === 'openverse' ? 'dishImageSourceOpenverse' : 'dishImageSourceGoogle');
            ...
            <div class="header-content">
                <div class="brand">
                    ...
                    <div class="header-left">
                        <h1>Kantinen Übersicht <small class="version-tag" style="font-size: 0.6em; opacity: 0.7; font-weight: 400; cursor: pointer;" title="Klick für Versionsmenü">v2.1.3</small></h1>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div style="margin-bottom: 1rem;">
                        <strong>Aktuell:</strong> <span id="version-current">v2.1.3</span>
                    </div>
...
function updateUILanguage() {
    ...
    if (dishImageHeader) dishImageHeader.textContent = (0,i18n.t)('dishImageModalTitle');
...
function bindEvents() {
    ...
    window.addEventListener('click', (e) => {
        if (dishImageModal && e.target === dishImageModal) (0,ui_helpers/* closeDishImageModal */.zf)();
    });
...
if (!window.__KANTINE_LOADED) {
    ...
    stats_tracker/* tracker */.F.incrementCategory('version', 'v2.1.3');
